...
export const BASE_URL = "https://price-indexer-api.vercel.app";
...
export type LiquidlinkDashboardResponse<T> = { code: number; message: string; data: T };
...
export class LiquidlinkDashboardClient {
  ...
  private async send<T>(method: Method, path: string, opts?: { query?: Record<string, any>; body?: any }) {
    ...
    if (res.statusCode < 200 || res.statusCode >= 300) {
      ...
      throw new Error(`[LiquidlinkDashboardSDK] ${message}`);
    }
    if (!text || parseError) {
      throw new Error(`Invalid JSON: ${text}`);
    }
    if (parsed && typeof parsed === "object" && "code" in parsed) {
      const p = parsed as LiquidlinkDashboardResponse<T>;
